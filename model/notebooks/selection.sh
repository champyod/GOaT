...
trap 'echo "[trap] selection.sh got TERM/INT at $(date -u +%FT%TZ)" >&2; exit 143' TERM INT
trap 'code=$?; echo "[trap] selection.sh exiting code=$code at $(date -u +%FT%TZ)" >&2; if [ $code -eq 137 ]; then echo "[trap] 137=SIGKILL: likely OOM-killed or VM preempted - check RAM (scripts/monitor_ram.py) and batch sizes" >&2; fi' EXIT
