...
watch -n 3 'tail -n 25 /tmp/goat_training_log.txt; echo ---; free -h | head -2; df -h / /tmp | tail -2; nvidia-smi --query-gpu=utilization.gpu,memory.used,temperature.gpu,power.draw --format=csv,noheader; ps -o pcpu,pmem,etime,args -p $(pgrep -f "train_mt|train_ocr" | head -5)'
```
...
Notes:
- Synthetic data runs fully local: Drive gen/fonts/corpus copy to /tmp/synth_* first, workers read /tmp only, background + final sync push results back to Drive.
